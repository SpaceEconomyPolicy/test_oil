...
Этот GitHub-репозиторий содержит **данные организатора, контрольные правила, форматы обмена, синтетические тест-векторы, научную базу и порядок проверки**. Здесь **нет готовой конкурсной стратегии**. Репозиторий не выбирает за команду source mix, объёмы закупки, момент инвестиций, метод оптимизации, программный стек или интерфейс.

> **Ключевой принцип:** мы стандартизируем задачу и проверку, а не конкурсное решение.
...
Задача — не нарисовать статичный dashboard и не написать текстовый отчёт, а связать управленческие решения с пересчитываемой моделью.
...
Годовая мощность Emergency 80 т/год **сама по себе не означает**, что 80 т находятся в узле или могут прибыть немедленно. Нужно учитывать 6-week lead time.
...
2041 **не является прогнозом организатора**. Исходные 2035–2040 limits не переносятся или отменяются молча.
...
1. запустить систему по README команды;
2. сверить несколько чисел с запиской/export;
3. выбрать один year/source и проследить input -> algorithm -> result -> constraint;
4. изменить одно допустимое решение без правки кода;
5. проверить пересчёт dependent outputs;
6. создать knowingly invalid plan;
7. получить конкретный violation;
8. запустить mandatory stress;
9. сравнить BASE/STRESS;
10. export;
11. save/reopen;
12. extension test на копии.

Расширенная версия: [`docs/JURY_CHECKLIST.md`](docs/JURY_CHECKLIST.md).

---

## 31. Карта критериев 100 + 5
...
## 32. Геополитический бонусный модуль
...
1. **Critical demand прибавлен к total ещё раз.**
2. **BASE delivery умножен на reliability.**
3. **Mandatory stress ISRU 55%/75% снова умножен на reliability.**
4. **Shortage представлен отрицательным physical inventory.**
5. **Одни и те же storage losses начислены дважды.**
6. **Take-or-pay начислен второй раз поверх `max(...)`.**
7. **Earth-New посчитан как `90 + 270 + 360`.**
8. **Emergency annual capacity выдана за немедленный 45-day reserve без 6-week lead time.**
9. **Capacity, budget или lead time изменены скрытно, чтобы сделать plan feasible.**
10. **Статический screenshot назван digital contour.**
11. **TEAM_ASSUMPTION выдан за organiser fact.**
12. **Реальные company prices заменили control case prices.**
13. **Точные probabilities придуманы без статистической базы.**
14. **Всё агрегировано только по году, поэтому внутригодовой shortage не виден.**
15. **UI и export используют разные calculation paths.**

---

## 34. Структура репозитория команды
...
## 37. Финальная проверка перед защитой

- [ ] Числа в записке, UI и export совпадают.
- [ ] BASE воспроизводится после чистого запуска.
- [ ] `MANDATORY_STRESS` содержит точные organiser multipliers.
- [ ] Один intentionally invalid plan выдаёт конкретный violation.
- [ ] Один manual arithmetic control case совпадает с системой.
- [ ] Critical demand нигде не прибавляется к total второй раз.
- [ ] BASE supply не умножается автоматически на reliability.
- [ ] ISRU 55%/75% не умножается на reliability повторно.
- [ ] Save/reopen работает.
- [ ] Export совпадает с UI.
- [ ] На копии набора протестирован synthetic source/future period.
- [ ] `TEAM_ASSUMPTION` отделены от `CASE_INPUT`.
- [ ] Scientific claims имеют traceable sources.
- [ ] В repository нет `.env`, API keys, passwords, tokens и private credentials.

Для самого starter kit организатора используется [`docs/RELEASE_CHECKLIST.md`](docs/RELEASE_CHECKLIST.md).
...
**ISRU** — in-situ resource utilization. В кейсе Lunar-ISRU — перспективный лунный источник с доставкой топлива в узел.

**ZBO** — zero boil-off. В учебном наборе ZBO modernization — условное название storage upgrade с model loss rate 1.2% throughput.
...
**MCDA** — multi-criteria decision analysis; при использовании раскрываются criteria, normalization и weights.
...
### Самопроверка starter repository

Static validator **не решает кейс**. Он проверяет только целостность starter kit: контрольные числа, mandatory stress, schemas, relative links, отсутствие готового participant application и типовые ошибки переноса данных.
